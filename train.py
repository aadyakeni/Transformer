...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer, num_examples=2):
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            print_msg(f'EXPECTED: {model_out_text}')
...
def get_ds(config):
    ...
    train_ds = BilingualDataset(train_ds_raw, tokenizers_src, tokenizers_tgt, config['lang_src'], config['lang_tgt'], config['seq_len'])
    val_ds = BilingualDataset(val_ds_raw, tokenizers_src, tokenizers_tgt, config['lang_src'], config['lang_tgt'], config['seq_len'])
